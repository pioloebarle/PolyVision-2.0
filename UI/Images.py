...
class ImagesUI(QDialog):
    ...
    def create_image_label(self, folder_path, image_path, image_name, row, col):
        ...
        def on_image_clicked(label):
            ...
            if data is not None and len(data) > 0:
                first_row = data[0]
                self.particle_name_label.setText("Name: " + str(first_row[1])) 
                
                # Check if the length value is not None before formatting
                try:
                    formatted_length = "{:.4f}".format(float(first_row[2])) if first_row[2] is not None and first_row[2] != '' else ""
                except (ValueError, TypeError):
                    formatted_length = ""
                
                # Check if the width value is not None before formatting
                try:
                    formatted_width = "{:.4f}".format(float(first_row[3])) if first_row[3] is not None and first_row[3] != '' else ""
                except (ValueError, TypeError):
                    formatted_width = ""
    ...
    def apply_filters(self, label):
        ...
        if data:
            ...
            #Check shape filters
            if (filaments_checked and shape == "Filament") or (fragments_checked and shape == "Fragment") or (films_checked and shape == "Film"):

                #Check filament
                if shape == "Filament":
                    if length_data is not None and length_data != '':
