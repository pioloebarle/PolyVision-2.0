...
class SettingsUI(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        theme_options = ["Binary", "Multiclass"]
        ...
        self.initial_settings = self.getCurrentSettings()
        
        # Initially disable Apply button (no modifications yet)
        self.apply_button.setEnabled(False)
        
        # Connect all input widgets to check for modifications
        self.resolution_dropbox.currentIndexChanged.connect(self.checkForModifications)
        self.sharpness_balance_slider.valueChanged.connect(self.checkForModifications)
        self.saturation_balance_slider.valueChanged.connect(self.checkForModifications)
        self.steps_line_edit.textChanged.connect(self.checkForModifications)
        self.feed_line_edit.textChanged.connect(self.checkForModifications)
        self.scan_overlay_checkbox.toggled.connect(self.checkForModifications)
        self.theme_dropbox.currentIndexChanged.connect(self.checkForModifications)
        self.sound_checkbox.stateChanged.connect(self.checkForModifications)
    ...
    def getCurrentSettings(self):
        ...
        return {
            "image_quality": self.resolution_dropbox.currentText(),
            ...
            "model": self.theme_dropbox.currentText(),
    ...
    def checkForModifications(self):
        ...
        has_modifications = current != self.initial_settings
        self.apply_button.setEnabled(has_modifications)
    ...
    def resetToDefault(self):
        ...
        # Update initial settings after reset
        self.initial_settings = self.getCurrentSettings()
        
        # Disable Apply button after reset
        self.apply_button.setEnabled(False)
    ...
    def applySettings(self):
        ...
        # Update initial settings to current state after applying
        self.initial_settings = self.getCurrentSettings()
