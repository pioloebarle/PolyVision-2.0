...
class BoundingBox():
    ...
    def __init__(self,file_path, plots, port, parent=None):
        ...
        # Apply the SAME resizing logic as the model does
        if original_size > 1200:
            scale_factor = 800 / original_size
            new_width = int(original_width * scale_factor)
            new_height = int(original_height * scale_factor)
            image = image.resize((new_width, new_height))
            print(f"BBox: Resized from {original_width}x{original_height} to {new_width}x{new_height} to match model processing")
